...
router = APIRouter(prefix="/okx/algo", tags=["OKX Algo Trading"])

def get_algo_service() -> OKXAlgoService:
    services = get_services()
    return services.okx_algo_service
...
async def place_tp_sl_order(
    request: OKXTPSLOrderRequest,
    algo_service: OKXAlgoService = Depends(get_algo_service)
):
    """
...
async def place_trigger_order(
    request: OKXTriggerOrderRequest,
    algo_service: OKXAlgoService = Depends(get_algo_service)
):
    """
...
async def place_trailing_stop_order(
    request: OKXTrailingStopRequest,
    algo_service: OKXAlgoService = Depends(get_algo_service)
):
    """
...
async def place_iceberg_order(
    request: OKXIcebergOrderRequest,
    algo_service: OKXAlgoService = Depends(get_algo_service)
):
    """
